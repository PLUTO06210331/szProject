<style lang='scss' scoped>
  ...
  .warehouseOut-wrap .specialHandling{
    width:100px;
    border:none;
  }
...
<template>
    <div class="warehouseOut-wrap spaceBetween" v-com>
        <div class="allShow white-bottom">
            ...
            <div class="tableDistance">
                <Form class="searchForm searchStyle" label-position="left" inline ref='formSearch' :model='searchForm'>
                    <Form-item label="出库方式" :label-width="80" class="specialCase" prop='transtype'>
                        <Select v-model="searchForm.mode">
                            <Option v-for="item in outWayList" :value="item.id" :key="item.id">{{item.name }}</Option>
                        </Select>
                    </Form-item>
                    <Form-item label="出库类型" :label-width="80" class="specialCase" prop='mode'>
                    ...
                    <Form-item label="出库时间" :label-width="80" class="specialCase" style='width:305px' prop='createDate'>
        ...
        <div v-if="LookIntoShow">
            <Modal v-model="LookIntoShow" :mask-closable='false'  width="770" v-drag class-name='lookIntoModal-wrap'>
                <p slot="header">
        ...
        <div v-if="InsertMatterIntoShow">
            <Modal v-model="InsertMatterIntoShow"
                   :rules='InsertMatterIntoRules'
                   ref='InsertMatterIntoRules'
                   class-name="delect big-sized"
                   :mask-closable='false'
                    :closable="false"
                   width="870">
                <p slot="header">
                    <span class="main-heading">物料出库</span>
                </p>
                <div class="modalContent" style="text-align:center">
                    <Form ref="MatterOutInfo" class="searchForm" label-position="left" :model="MatterOutInfo" :rules='MatterOutInfoRules' inline style="text-align: left;padding:15px 20px 0">
                        <Form-item prop="code"  label="出库单号:" :label-width="70">
                            <Input v-model="MatterOutInfo.code" disabled></Input>
                        </Form-item>
                        <Form-item prop="shopName"  label="门店:" :label-width="70">
                            <Input v-model="shopName" disabled></Input>
                        </Form-item>
                        <Form-item prop="transtype"  label="出库类型:" :label-width="70" >
                            <Select v-model="MatterOutInfo.transtype">
                                <Option v-for="item in outTypeList" :value="item.id" :key="item.id">{{item.name}}</Option>
                            </Select>
                        </Form-item>
                        <Form-item prop="storageDate"  label="出库时间:" :label-width="70" >
                            <DatePicker type="datetime" v-model='MatterOutInfo.storageDate' class='matterInto-timeWrap'></DatePicker>
                        </Form-item>
                        <Form-item prop="remark"  label="说明:" :label-width="70" class='form-item-block' style='width:95%'>
                            <Input v-model="MatterOutInfo.remark"></Input>
                        </Form-item>
                    </Form>
                    ...
                    <div class="styleDetail" style='padding:0 20px'>
                        <Table border height="300" :columns="MatterIntocolumns" :data="MatterOutInfo.list"></Table>
                    </div>
                </div>
                <div slot='footer'>
                    <Button  class="save" @click="insertMatterOK('MatterOutInfo')">确认</Button>
                    <Button  class="clear" @click="InsertMatterCancels('MatterOutInfo')">取消</Button>
                </div>
            </Modal>
        </div>
        <!-- 订单号出库 -->
        <div v-if="InsertOrderIntoShow">
            <Modal v-model="InsertOrderIntoShow" class-name="delect big-sized" :mask-closable='false'  width="700" v-drag @on-cancel="cancels('MatterOutInfo')">
                <p slot="header">
                    <span class="main-heading">订单号出库</span>
                </p>
                <div class="modalContent" style="text-align:center">
                    <Form ref="OrderIntoInfo" class="searchForm" label-position="left" :model="OrderIntoInfo" inline style="text-align: left;padding:15px 20px 0">
                        <Form-item prop="intoNumber"  label="出库单号:" :label-width="90" >
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                InsertMatterIntoRules:{
                    code:[
                        {requied:true,message:'出库单号不能为空',trigger:'blur'}
                    ],
                    shopName:[
                        {requied:true,message:'门店不能为空',trigger:'blur'}
                    ],
                    transtype:[
                        {requied:true,message:'出库类型不能为空',trigger:'blur'}
                    ],
                    storageDate:[
                        {requied:true,message:'出库时间不能为空',trigger:'blur'}
                    ]
                },
                shopName:'',
                MatterIntocolumns:[
                        ...
                        render:(h,params) => {
                            return h('div',[
                                h('InputNumber',{
                                    props:{
                                        min:0,
                                        value: this.MatterOutInfo.list[params.index].amount
                                    },
                                    on: {
                                        'on-change':(val) =>{
                                           this.MatterOutInfo.list[params.index].amount = val;
                                        }
                                    },
                                    style:{
                                        width:'57px',
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            return h('Select', {
                                props:{
                                    value: this.MatterOutInfo.list[params.index].warehouseid,
                                    'label-in-value':true
                                },
                                on: {
                                    'on-change':(val) => {
                                        this.MatterOutInfo.list[params.index].warehouseid = val.value;
                                        this.MatterOutInfo.list[params.index].warehouse = val.label;
                    ...
                    },
                    {
                        ...
                        render:(h,params) => {
                            return h('Select', {
                                props:{
                                    value:this.MatterOutInfo.list[params.index].location,
                                    'label-in-value':true
                                },
                                on:{
                                    'on-change':(val) => {
                                        this.MatterOutInfo.list[params.index].storagelocationid = val.label;
                                        this.MatterOutInfo.list[params.index].location = val.value;
                                    }
                                }
                            },
                            this.MatterOutInfo.locationList[params.index].map(function(item){
                                return h('Option',{
                                    props:{
                                        value:item.id,
                                        label:item.name
        ...
        mounted() {
            this.getOutstorageWay();
            this.getOutstorageType();
        ...
        methods: {
            ...
            getNowFormatDate() {
             ...
             // 搜索-出库方式
            getOutstorageWay(){
                this.$http.get('/outstorage/mode').then((res) => {
                    this.outWayList = res.data;
                })
            },
            // 搜索-出库类型
            getOutstorageType(){
                this.$http.get('/outstorage/type').then((res) => {
            ...
            outPut(){
                ...
                this.$http.post('/outstorage/export',this.searchForm).then((res) => {
            ...
            getStoreInfo(){
                this.$http.get('/store/info')
                        .then((res) => {
                            this.MatterOutInfo.storeid = res.data.id;
                            this.MatterOutInfo.id = res.data.id;
                            this.shopName = res.data.name;
                            this.MatterOutInfo.storecode = res.data.code;
                        })
            },
            // 物料入库-主表格选中某一行
            ...
            insertMatterModalShow(){
                // 物料出库-出库单号
                this.$http.get('/instorage/outcode').then((res) => {
                    this.MatterOutInfo.code = res.data;
                    this.InsertMatterIntoShow=true;
                    this.MatterOutInfo.storageDate = this.getNowFormatDate();
                })
            },
            // 物料出库-出库列表
            getOutstorageList(){
                if (this.searchForm.createDate[0]) {
                    this.searchForm.beginTime = this.setDate(this.searchForm.createDate[0]);
                    this.searchForm.endTime = this.setDate(this.searchForm.createDate[1]);
                } else {
                    this.searchForm.beginTime = '';
                ...
                this.$http.get('/outstorages',{params:this.searchForm}).then((res) => {
                    let resData = res.data;
            ...
            insertMatterOK(form){
                 this.$refs[form].validate((valid) => {
                     if (valid) {
                         let len = this.MatterOutInfo.list.length;
                        if(len>=1){
                            let bool = this.MatterOutInfo.list.every((item) => {
                                return item.warehouseid
                            })
                            if(bool){
                                this.$http.post('/outstorage/material',this.MatterOutInfo)
                                .then((res) => {
                                    this.MatterOutInfo.locationList = [];
                                    this.MatterOutInfo.list = [];
            ...
            InsertMatterCancels(form){
                let len = this.MatterOutInfo.locationList.length;
                this.MatterOutInfo.locationList.splice(0,len);
                this.MatterOutInfo.list = [];
                this.$refs[form].resetFields();
                this.InsertMatterIntoShow = false;
            },
            ...
            warehouseSelect(val,index){
                this.$http.get('/storagelocation/list',{params:{
                  warehouseid:val.value
                    ...
                    this.$set(this.MatterOutInfo.locationList,index,res.data);
                })
            },
            // 分页
            pageListen(data){
            ...
            // 物料出库-删除表格中的某一项
            remove(index){
                this.MatterOutInfo.list.splice(index,1);
                if(this.MatterOutInfo.list.length<1){
                    this.MatterOutInfo.locationList = [];
                }
            },
            /** ==================== 新增物料 ================== **/
            ...
            addMaterielOK(form){
                ...
                this.addMaterielTableSelection.map((item,index) => {
                    ...
                    this.$set(this.MatterOutInfo.list,index,o);
                    this.MatterOutInfo.locationList.push([]);
                });
